...
[~, ~] = coreMetRxnAnalysis([], model, coreMetAbbr, coreRxnAbbr, [], [], param);
...
if (isfield(specificData, 'rxns2remove') && ~isempty(specificData.rxns2remove))
    ...
    if param.activeOverInactive && sum(ismember(specificData.rxns2remove.rxns, coreRxnAbbr)) > 1
    ...
    elseif ~param.activeOverInactive && sum(ismember(coreRxnAbbr, specificData.rxns2remove.rxns)) > 1
