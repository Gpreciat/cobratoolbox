function sampleFig = plotSampleHist(rxnNames, samples, models, nBins, perScreen, options)
...
while ~flagQuit
    ...
    if ~isempty(FVAdata)
        ...
        xlim = get(gca, 'xlim');
        dataL = [];
        for i = 1:size(FVAdata, 2)
            eval(sprintf('p%s = plot(repmat(FVAdata(id, i),1,2), ylim, ''--'', ''Color'', plotColor(i, :));', num2str(i)));
            if mod(i, 2) == 1
                text(FVAdata(id, i), (ylim(2) / 2) + ((ylim(2) / 20) * i), 'min', 'Color', plotColor(i, :));
                eval(sprintf('dataL = [dataL p%s];', num2str(i)));
            else
...
%     pl = plot(bins, freq, '-');
%     axis([currLB - .0001 currUB + .0001 0 max(max(freq))]);
    if j==1 && (length(models) > 1 || (isfield(options, 'FVAdata')))
        if (isfield(options, 'FVAdata'))
            legend(dataL, legends)
        else
            legend(legends) 
        end
    end
